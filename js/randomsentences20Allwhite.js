...
function everyFresh() {
    for (var num = 0; num < ul.length; num++) {
        ...
        //定义字体大小并随机写入文字
        var rand = GetRandomNum(0, list.length);
        var len = list[rand].length;
        var fontSZ = 180 / Math.pow(len, 0.5);
        li[num].style.fontSize = fontSZ + "px";
        li[num].innerHTML = list[rand];
    }
...
function changeOnePiece(n) {
    ...
    var len = list[rand].length;
    var fontSZ = 180 / Math.pow(len, 0.5);
    li[n].style.fontSize = fontSZ + "px";
    li[n].innerHTML = list[rand];
}
