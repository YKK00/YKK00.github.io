...
function everyFresh() {
    ...
    for (var num = 0; num < ul.length; num++) {
        ul[num].style.background = "white";//设置ul背景颜色
        li[num].style.background = "white";//设置li背景颜色
        switch (num) {
            case (0):
                i = GetRandomNum(0, list1.length);
                var len = list1[i].length;
                var fontSZ = 400 / Math.pow(len, 0.5);
                li[0].style.fontSize = fontSZ + "px";
                li[0].innerHTML = list1[i];
                break;
            case (1):
                i = GetRandomNum(0, list2.length);
                var len = list2[i].length;
                var fontSZ = 400 / Math.pow(len, 0.5);
                li[1].style.fontSize = fontSZ + "px";
                li[1].innerHTML = list2[i];
                break;
            case (2):
                i = GetRandomNum(0, list3.length);
                var len = list3[i].length;
                var fontSZ = 400 / Math.pow(len, 0.5);
                li[2].style.fontSize = fontSZ + "px";
                li[2].innerHTML = list3[i];
                break;
            case (3):
                i = GetRandomNum(0, list4.length);
                var len = list4[i].length;
                var fontSZ = 400 / Math.pow(len, 0.5);
                li[3].style.fontSize = fontSZ + "px";
